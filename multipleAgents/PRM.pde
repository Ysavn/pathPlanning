...
ArrayList<Integer> planPath(Vec2 startPos, Vec2 goalPos, Vec2[] centers, float[] radii, int numObstacles, Vec2[] nodePos, int numNodes, Vec2 otherPos){
  ArrayList<Integer> path = new ArrayList();
  allStartIDs.clear();
  
  Vec2 dir = goalPos.minus(startPos).normalized();
  float distBetween = goalPos.distanceTo(startPos);
  boolean circleListCheck = rayCircleListIntersect(centers, radii, numObstacles, startPos, dir, distBetween).hit || 
  rayCircleIntersect(otherPos, ra, startPos, dir, distBetween).hit;
  //no collisons in direct path from startPos to goalPos
  if(!circleListCheck) return path;
  ...
  for(int i=0;i<numNodes;i++)
  {
    ...
    circleListCheck = rayCircleListIntersect(centers, radii, numObstacles, startPos, dir, distBetween).hit ||
    rayCircleIntersect(otherPos, ra, startPos, dir, distBetween).hit;
    if(!circleListCheck){
      allStartIDs.add(i);
    }
  ...
  for(int i=0;i<numNodes;i++)
  {
    ...
    circleListCheck = rayCircleListIntersect(centers, radii, numObstacles, endNode, dir, distBetween).hit ||
    rayCircleIntersect(otherPos, ra, startPos, dir, distBetween).hit;
    if(!circleListCheck){
...
void connectNeighbors(Vec2[] centers, float[] radii, int numObstacles, Vec2[] nodePos, int numNodes){
  for (int i = 0; i < numNodes; i++){
    ...
    for (int j = 0; j < numNodes; j++){
      ...
      boolean circleListCheck = rayCircleListIntersect(centers, radii, numObstacles, nodePos[i], dir, distBetween).hit || 
      rayCircleIntersect(startPos, ra, nodePos[i], dir, distBetween).hit;
      if (!circleListCheck){
        neighbors[i].add(j);
      }
    }
